...
use ethabi::{Hash as EthHash, Log, RawLog};
use serde_json::Value as JSONValue;

use crate::abi::{ABIError, ABI};
            ...
            .map(|log| {
                let topics = match log["topics"].as_array() {
                    Some(topics) => topics
                        ...
                        .map(|topic| {
                            EthHash::from_slice(
                                hex::decode(topic.to_owned().trim_start_matches("0x"))
                                    .unwrap_or(vec![])
                                    .as_slice(),
                            )
                        })
...
pub fn parse_event_log(
    ...
    let abi = ABI::new(&Some(abi_content), &None, "", sm_crypto)?;
    ...
    fn test_parse_event_log() {
        let expected_logs = vec![Log {
            params: vec![
                LogParam {
                    name: "s".to_string(),
                    value: Token::String("1234".to_string()),
                },
                LogParam {
                    name: "n".to_string(),
                    value: Token::Int(0.into()),
                },
            ],
        }];
