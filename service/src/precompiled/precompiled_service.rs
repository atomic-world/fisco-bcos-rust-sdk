...
use crate::{
    abi::{ABIError, ABI},
    helpers::parse_json_string,
    web3::service::{CallResponse, Service as Web3Service, ServiceError as Web3ServiceError},
};
...
pub(crate) fn parse_output(output: &Int) -> Result<i32, PrecompiledServiceError> {
    ...
    let message = match code {
        ...
        ACCOUNT_NOT_EXIST => "Account not exist, you can create a blockchain account by using \
                              this account to deploy contracts on the chain"
            .to_owned(),
        ...
        OPERATOR_CANNOT_BE_COMMITTEE_MEMBER => {
            "The operator or cnsManager cannot be committee member".to_owned()
        }
        ...
        INVALID_REVOKE_LAST_AUTHORIZATION => "There is only one contract status manager left, and \
                                              the revoke operation cannot be performed"
            .to_owned(),
        ...
        INVALID_CONTRACT_REPEAT_AUTHORIZATION => {
            "The contract has been granted authorization with same user".to_owned()
        }
        ...
        COMMITTEE_PERMISSION => "The committee permission control by ChainGovernancePrecompiled \
                                 are recommended"
            .to_owned(),
        ...
        TABLE_FILED_TOTAL_LENGTH_OVERFLOW => {
            "The length of all the fields name exceeds the limit 1024".to_owned()
        }
        TABLE_KEY_VALUE_LENGTH_OVERFLOW => "The value exceeds the limit, key max length is 255, \
                                            field value max length is 1024"
            .to_owned(),
...
pub(crate) async fn send_transaction(
    ...
    let transaction_hash = web3_service
        .send_transaction_with_abi("sendRawTransaction", address, &abi, method, &tokens)
        .await?;
    let transaction_receipt = web3_service
    ...
    if transaction_receipt.is_null() {
        return Err(PrecompiledServiceError::CustomError {
            message: format!(
                "Transaction invoked, but the action for fetching transaction receipt is timeout. \
                 Transaction hash is {:?}",
                transaction_hash
            ),
        });
    ...
    let tokens = abi
        .decode_output(method, &parse_json_string(&transaction_receipt["output"]))?
        .unwrap();
...
pub(crate) async fn call(
    ...
    let tokens = abi.parse_function_tokens(method, &params)?;
    Ok(web3_service
        .call_with_abi(address, &abi, method, &tokens)
        .await?)
}

pub(crate) fn parse_string_token_to_json(
...
pub(crate) fn parse_address_token_to_string(
    tokens: &Option<Vec<Token>>,
