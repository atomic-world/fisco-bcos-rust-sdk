...
use sqlparser::{
    ast::{
        Assignment, BinaryOperator, ColumnDef, Expr, Ident, ObjectName, Query, SelectItem, SetExpr,
...
use crate::{
    precompiled::{
        ...
        table_crud_service::{Condition, ConditionOperator, TableCRUDService},
    },
...
impl<'l> SQLService<'l> {
    ...
    fn get_key_field_value_from_condition(
        ...
        let err = Err(PrecompiledServiceError::CustomError {
            message: format!(
                "Value of key field {:?} should be provided in where clause",
                key_field
            ),
        });
    ...
    fn validate_fields(
        ...
        let invalid_fields: Vec<String> = expected_fields
            .clone()
            .into_iter()
    ...
    fn parse_condition(&self, selection: &Expr, condition: &mut Condition) {
        match selection {
            Expr::BinaryOp { left, op, right } => {
                // 因多个 where 条件之间仅支持 AND 操作符且单个 where 条件仅包含
                // =、!=、>、<、>=、<= 操作符。 故此处可以断定，如果操作符为 AND
                // 时即表示多个 where 条件的连接（比如 name = "Tom" and age = "18"），
    ...
    async fn execute_create_table(
        ...
        let fields: Vec<String> = columns
            ...
            .collect();
        let table_crud_service = TableCRUDService::new(self.web3_service);
        Ok(table_crud_service
            .create_table(&table_name, &key_fields[0], &fields)
            .await?)
    }
    ...
    async fn execute_insert(
        ...
        if value_length != value_field_length {
            return Err(PrecompiledServiceError::CustomError {
                message: format!(
                    "Unmatched number of values, expected {:?} but got {:?}",
                    value_field_length, value_length
                ),
            });
        }
        ...
        for (index, value_field) in value_fields.into_iter().enumerate() {
            let value = self
                .get_value_from_expr(&values[index])
        ...
        if key_field_value.is_empty() {
            ...
            let table_crud_service = TableCRUDService::new(self.web3_service);
            Ok(table_crud_service
                .insert(&table_name, &key_field_value, &entry)
                .await?)
        }
    }
    ...
    async fn execute_query(&self, query: &Query) -> Result<JSONValue, PrecompiledServiceError> {
        if let SetExpr::Select(select) = &query.body {
            ...
            if table_name.len() == 0 {
                return Err(PrecompiledServiceError::CustomError {
                    message: "Can't parse table name with the invalid select sql statement"
                        .to_string(),
                });
            }
            // 表结构获取
                ...
                _ => projection
                    ...
                    .map(|item| match item {
                        SelectItem::UnnamedExpr(expr) => {
                            self.get_value_from_expr(&expr).unwrap_or(String::from(""))
                        }
                        _ => String::from(""),
                    })
                    .filter(|v| v.len() > 0)
            ...
            let key_field_value = self.parse_selection(
                &table_name,
                &key_field,
                &table_fields,
                &select.selection,
            ...
            let offset = match &query.offset {
                Some(offset) => self
                    .get_value_from_expr(&offset.value)
                    .map(|v| v.parse::<i32>().unwrap_or(-1))
                    .unwrap_or(-1),
                None => -1,
            };
            ...
            // 数据获取
            let table_crud_service = TableCRUDService::new(self.web3_service);
            let records = table_crud_service
                .select(&table_name, &key_field_value, &condition)
                .await?;
            // 对数据进行解析，只返回指定字段的数据
                ...
                Ok(json!(records
                    ...
                    .map(|record| {
                        ...
                        for select_field in &select_fields {
                            result[select_field.to_owned()] =
                                record.get(select_field).unwrap_or(&json!(null)).clone();
                        }
                        result
                    })
    ...
    async fn execute_delete(
        ...
        let table_crud_service = TableCRUDService::new(self.web3_service);
        Ok(table_crud_service
            .remove(&table_name, &key_field_value, &condition)
            .await?)
    }
    ...
    async fn execute_update(
        ...
        let table_crud_service = TableCRUDService::new(self.web3_service);
        Ok(table_crud_service
            .update(&table_name, &key_field_value, &entry, &condition)
            .await?)
    }

    pub fn new(web3_service: &'l Web3Service) -> SQLService<'l> {
        SQLService { web3_service }
    }
        ...
        match &ast[0] {
            ...
            Statement::Insert {
                ...
                if let SetExpr::Values(values) = &source.body {
                    Ok(json!(
                        self.execute_insert(table_name, columns, &(values.0)[0])
                            .await?
                    ))
                } else {
                    Err(PrecompiledServiceError::CustomError {
            ...
            Statement::Update {
                ...
                self.execute_update(table_name, assignments, selection)
                    .await?
            )),
