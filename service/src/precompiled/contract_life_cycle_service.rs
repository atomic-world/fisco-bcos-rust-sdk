use crate::{
    precompiled::precompiled_service::{
        call, parse_output, send_transaction, PrecompiledServiceError,
    },
...
impl<'l> ContractLifeCycleService<'l> {
    pub fn new(web3_service: &'l Web3Service) -> ContractLifeCycleService<'l> {
        ContractLifeCycleService { web3_service }
    }
    ...
    pub async fn get_status(
        &self,
        contract_address: &str,
    ) -> Result<(i32, String), PrecompiledServiceError> {
        let params = vec![contract_address.to_owned()];
        let response = call(
            self.web3_service,
            "ContractLifeCyclePrecompiled",
    ...
    pub async fn list_manager(
        &self,
        contract_address: &str,
    ) -> Result<(i32, Vec<String>), PrecompiledServiceError> {
        let params = vec![contract_address.to_owned()];
        let response = call(
            self.web3_service,
            "ContractLifeCyclePrecompiled",
